...
export const propertyPrompts: { [key: string]: string } = {
    ...
    useCases: "Enhance the provided use case by following industry-standard practices. Structure the use case as follows:\n\n1. Title: A clear, concise title for the use case.\n2. Actors: Identify the primary and secondary actors involved.\n3. Preconditions: List any conditions that must be true before the use case begins.\n4. Main Success Scenario: Describe the step-by-step flow of events for the successful path.\n5. Alternative Scenarios: Include any alternative paths or variations from the main scenario. For each, specify:\n   - Name\n   - Start point (step number in the main scenario)\n   - Steps\n   - Return point (step number in the main scenario or 'end')\n6. Error Scenarios: Describe potential error conditions and how they are handled. For each, specify:\n   - Name\n   - Start point (step number in the main scenario)\n   - Steps\n7. Postconditions: State the system's condition after the use case is completed.\n8. UI Requirements: List any specific user interface requirements for this use case.\n\nEnsure the use case is detailed, specific, and provides clear value to the development team. Use simple, easy-to-understand language. Maintain the language and language style of the input data. Don't give any explanations. Use H3 for headings.",
    functionalRequirements: `Enhance the provided functional requirements based on the following project details. Each requirement should include:

    - **Requirement ID**: A unique identifier for the requirement.
    - **Title**: A brief, descriptive title summarizing the requirement.
    - **Requirement**: A detailed description of the functionalities that the system should provide. Create a list of sub-requirements that each start with "The system shall ...". Ensure the sub-requirements are clear, concise, and free of ambiguity.
    ...
    Make sure all of these properties are written on separate lines. Use the language of the project details to write the functional requirements. Order them from most important to least important. Make sure they are detailed and clear. If the input is too short or missing key points, add suggestions to make a complete list. If the requirements can be made more granular by splitting them up, please do so. Use plain language that anyone can understand. Format the output as a Markdown list, with each requirement starting on a new line and using only H3 for headings.
    `,
